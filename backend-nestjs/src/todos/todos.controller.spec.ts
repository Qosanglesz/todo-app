...
const mockTodosService = {
  ...
  findAll: jest
    .fn()
    .mockResolvedValue({ data: [mockTodo], totalCount: 1, totalPages: 1 }),
...
describe('TodosController', () => {
  ...
  it('should get one todo', async () => {
    ...
    expect(mockTodosService.findOne).toHaveBeenCalledWith(
      'todo123',
      mockUser.userId,
    );
  });
  ...
  it('should delete a todo', async () => {
    ...
    expect(mockTodosService.remove).toHaveBeenCalledWith(
      'todo123',
      mockUser.userId,
    );
  });
