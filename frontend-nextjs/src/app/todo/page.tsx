...
export default function TodoPage() {
  ...
  const onEdit = async (todoId: string, newData: TodoUpdateDTO) => {
    ...
    try {
      ...
      if (result.success && result.updatedTodo) {
        ...
        setTodos((prevTodos) =>
          prevTodos.map((todo) => (todo.todo_id === updatedTodo.todo_id ? updatedTodo : todo)),
        );
  ...
  return (
    <div className="h-screen">
      ...
      <div className="flex flex-col justify-between items-center my-5 sm:flex-row sm:justify-between sm:items-start">
        <div className="w-1/3">
